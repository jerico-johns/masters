...
def calculate_top_n(row, n):
    ...
    for col in ['tier_1_1_score', 'tier_1_2_score', 'tier_1_3_score',
        ...
        try:
            # Replace NaN with 99 before converting to int
            score = 99 if pd.isna(row[col]) else int(row[col])
            scores.append(score)
        except (ValueError, KeyError):
            scores.append(99)  # Use 99 as a default high score for any errors
